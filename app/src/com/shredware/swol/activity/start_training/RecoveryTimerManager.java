...
package com.shredware.swol.activity.start_training;
			...
			public void run() {
				...
				if (counter < max) {
					...
					if((((max/10) - (counter/10)) < 6) && (doVibrate || doBeep) && !setChannelIDFlag){
						...
						notificationManager.createNotificationChannel(mChannel);
						mNotificationBuilder = new NotificationCompat.Builder(mContext, CHANNEL_ID);
						mNotificationBuilder.setContentTitle("Get ready ...")
                                .setSmallIcon(R.drawable.icon_dumbbell_small)
                                .setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(),
                                        R.drawable.icon_dumbbell));
					...
					// final 1 seconds countdown
					if((((max/10) - (counter/10)) < 2) && (doVibrate || doBeep)){
						mNotifyManager.deleteNotificationChannel(CHANNEL_ID);
						CHANNEL_ID = rand.generateString();
						NotificationChannel mChannel = new NotificationChannel(CHANNEL_ID, name, importance);
						mChannel.setDescription(Description);
						...
						if(doBeep){
							...
							mChannel.setSound(uri, attrs.build());
						...
						mNotificationBuilder.setContentTitle("PUMP !!!")
                                .setSmallIcon(R.drawable.icon_dumbbell_small)
                                .setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(),
                                        R.drawable.icon_dumbbell));
					}
					...
					if(doVibrate || doBeep) {
						...
						if (doVibrate) {
							mChannel.setVibrationPattern(new long[]{000, 300});
						}
